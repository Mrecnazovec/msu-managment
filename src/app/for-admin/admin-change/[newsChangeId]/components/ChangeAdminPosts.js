...
import { checkPostsForAdmin, updatePostsForAdminChange } from '@/app/_actions/postActions'
...
const ChangeAdminPosts = ({ data }) => {
	...
	const [initialName, setInitialName] = useState(data[0].initialName)
	...
	const [roleLevel, setRoleLevel] = useState(data[0].roleLevel)
	...
	const handleSubmitTwo = async (e) => {
		...
		try {
			...
			const object = { id, fullName, password, modificator, role, login, gender, path, roleLevel, initialName }

			const res = await updatePostsForAdminChange(object)
...
export default ChangeAdminPosts
