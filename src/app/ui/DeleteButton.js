	...
	return (
		<>
			<button
				...
				className={`newsShow ${status === 'authenticated' && info.roleLevel >= 2 ? '' : 'none'} ${confirm ? 'none' : 'active'}`}
			>
				Удалить
			</button>
			...
			<button
				onClick={deleteNews}
				className={`newsShow ${status === 'authenticated' && info.roleLevel >= 2 ? '' : 'none'} ${confirm ? 'active' : 'none'} ${
					isConfirmed ? 'none' : ''
				}`}
			>
				Вы уверены?
